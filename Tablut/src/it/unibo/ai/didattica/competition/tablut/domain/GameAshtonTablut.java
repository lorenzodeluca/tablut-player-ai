...
public class GameAshtonTablut implements Game, aima.core.search.adversarial.Game<State, Action, State.Turn> {
	...
	public State checkMove(State state, Action a)
		...
		if(MatrixConfigs.DEFAULT_CLIENT_FILE_LOGGER)this.loggGame.fine(a.toString());
		// controllo la mossa
		if (a.getTo().length() != 2 || a.getFrom().length() != 2) {
			if(MatrixConfigs.DEFAULT_CLIENT_FILE_LOGGER)this.loggGame.warning("Formato mossa errato");
		...
		if (columnFrom > state.getBoard().length - 1 || rowFrom > state.getBoard().length - 1
			...
			if(MatrixConfigs.DEFAULT_CLIENT_FILE_LOGGER)this.loggGame.warning("Mossa fuori tabellone");
			throw new BoardException(a);
		}

		// controllo che non vada sul trono
		if (state.getPawn(rowTo, columnTo).equalsPawn(State.Pawn.THRONE.toString())) {
			if(MatrixConfigs.DEFAULT_CLIENT_FILE_LOGGER)this.loggGame.warning("Mossa sul trono");
			throw new ThroneException(a);
		}

		// controllo la casella di arrivo
		if (!state.getPawn(rowTo, columnTo).equalsPawn(State.Pawn.EMPTY.toString())) {
			if(MatrixConfigs.DEFAULT_CLIENT_FILE_LOGGER)this.loggGame.warning("Mossa sopra una casella occupata");
		...
		if (this.citadels.contains(state.getBox(rowTo, columnTo))
			...
			if(MatrixConfigs.DEFAULT_CLIENT_FILE_LOGGER)this.loggGame.warning("Mossa che arriva sopra una citadel");
		...
		if (rowFrom == rowTo && columnFrom == columnTo) {
			if(MatrixConfigs.DEFAULT_CLIENT_FILE_LOGGER)this.loggGame.warning("Nessuna mossa");
		...
		if (state.getTurn().equalsTurn(State.Turn.WHITE.toString())) {
			if (!state.getPawn(rowFrom, columnFrom).equalsPawn("W")
				...
				if(MatrixConfigs.DEFAULT_CLIENT_FILE_LOGGER)this.loggGame.warning("Giocatore " + a.getTurn() + " cerca di muovere una pedina avversaria");
		...
		if (rowFrom != rowTo && columnFrom != columnTo) {
			if(MatrixConfigs.DEFAULT_CLIENT_FILE_LOGGER)this.loggGame.warning("Mossa in diagonale");
				...
				for (int i = columnTo; i < columnFrom; i++) {
					if (!state.getPawn(rowFrom, i).equalsPawn(State.Pawn.EMPTY.toString())) {
						if (state.getPawn(rowFrom, i).equalsPawn(State.Pawn.THRONE.toString())) {
							if(MatrixConfigs.DEFAULT_CLIENT_FILE_LOGGER)this.loggGame.warning("Mossa che scavalca il trono");
							throw new ClimbingException(a);
						} else {
							if(MatrixConfigs.DEFAULT_CLIENT_FILE_LOGGER)this.loggGame.warning("Mossa che scavalca una pedina");
							throw new ClimbingException(a);
						}
					}
					if (this.citadels.contains(state.getBox(rowFrom, i))
							&& !this.citadels.contains(state.getBox(a.getRowFrom(), a.getColumnFrom()))) {
						if(MatrixConfigs.DEFAULT_CLIENT_FILE_LOGGER)this.loggGame.warning("Mossa che scavalca una citadel");
		...
		if (this.movesWithutCapturing == 0) {
			...
			if(MatrixConfigs.DEFAULT_CLIENT_FILE_LOGGER)this.loggGame.fine("Capture! Draw cache cleared!");
		}
		...
		for (State s : drawConditions) {
			...
			if (s.equals(state)) {
				...
				if (trovati > repeated_moves_allowed) {
					...
					if(MatrixConfigs.DEFAULT_CLIENT_FILE_LOGGER)this.loggGame.fine("Partita terminata in pareggio per numero di stati ripetuti");
					break;
				}
			} else {
				// DEBUG: //
		...
		if (trovati > 0) {
			if(MatrixConfigs.DEFAULT_CLIENT_FILE_LOGGER)this.loggGame.fine("Equal states found: " + trovati);
		}
		...
		if(MatrixConfigs.DEFAULT_CLIENT_FILE_LOGGER)this.loggGame.fine("Current draw cache size: " + this.drawConditions.size());

		if(MatrixConfigs.DEFAULT_CLIENT_FILE_LOGGER)this.loggGame.fine("Stato:\n" + state.toString());
	...
	private State checkCaptureWhite(State state, Action a) {
		...
		if (a.getColumnTo() < state.getBoard().length - 2
			...
			if(MatrixConfigs.DEFAULT_CLIENT_FILE_LOGGER)this.loggGame.fine("Pedina nera rimossa in: " + state.getBox(a.getRowTo(), a.getColumnTo() + 1));
		}
		...
		if (a.getRowTo() > 1 && state.getPawn(a.getRowTo() - 1, a.getColumnTo()).equalsPawn("B")
			...
			this.movesWithutCapturing = -1;
			if(MatrixConfigs.DEFAULT_CLIENT_FILE_LOGGER)this.loggGame.fine("Pedina nera rimossa in: " + state.getBox(a.getRowTo() - 1, a.getColumnTo()));
		}
		...
		if (a.getRowTo() == 0 || a.getRowTo() == state.getBoard().length - 1 || a.getColumnTo() == 0
			...
			if (state.getPawn(a.getRowTo(), a.getColumnTo()).equalsPawn("K")) {
				...
				if(MatrixConfigs.DEFAULT_CLIENT_FILE_LOGGER)this.loggGame.fine("Bianco vince con re in " + a.getTo());
			}
		}
		...
		if (a.getColumnTo() > 1 && state.getPawn(a.getRowTo(), a.getColumnTo() - 1).equalsPawn("K")) {
			...
			if (state.getBox(a.getRowTo(), a.getColumnTo() - 1).equals("e5")) {
				if (state.getPawn(3, 4).equalsPawn("B") && state.getPawn(4, 3).equalsPawn("B")
					...
					if(MatrixConfigs.DEFAULT_CLIENT_FILE_LOGGER)this.loggGame
	...
	private State checkCaptureBlackPawnRight(State state, Action a) {
		...
		if (a.getColumnTo() < state.getBoard().length - 2
			...
			if (state.getPawn(a.getRowTo(), a.getColumnTo() + 2).equalsPawn("B")) {
				state.removePawn(a.getRowTo(), a.getColumnTo() + 1);
				this.movesWithutCapturing = -1;
				if(MatrixConfigs.DEFAULT_CLIENT_FILE_LOGGER)this.loggGame.fine("Pedina bianca rimossa in: " + state.getBox(a.getRowTo(), a.getColumnTo() + 1));
			}
	...
	private State checkCaptureBlackPawnUp(State state, Action a) {
		...
		if (a.getRowTo() > 1 && state.getPawn(a.getRowTo() - 1, a.getColumnTo()).equalsPawn("W")
			...
			state.removePawn(a.getRowTo() - 1, a.getColumnTo());
			this.movesWithutCapturing = -1;
			if(MatrixConfigs.DEFAULT_CLIENT_FILE_LOGGER)this.loggGame.fine("Pedina bianca rimossa in: " + state.getBox(a.getRowTo() - 1, a.getColumnTo()));
		}
		return state;
	}
	...
	private State movePawn(State state, Action a) {
		...
		if(MatrixConfigs.DEFAULT_CLIENT_FILE_LOGGER)this.loggGame.fine("Movimento pedina");
